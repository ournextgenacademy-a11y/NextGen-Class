...
export const FormBuilder: React.FC = () => {
  ...
  // Edit Form Settings Modal State
  const [showEditFormModal, setShowEditFormModal] = useState(false);
  const [editFormTitle, setEditFormTitle] = useState('');
  const [editFormProgramId, setEditFormProgramId] = useState('');
  const [editFormCohortId, setEditFormCohortId] = useState('');
  ...
  const handleOpenEditFormModal = () => {
    ...
    setEditFormTitle(activeForm.title);
    setEditFormProgramId(activeForm.programId || (activeForm as any).programmeId || programs[0]?.id || '');
    setEditFormCohortId(activeForm.cohortId || '');
    setEditFormDescription(activeForm.description || '');
    setShowEditFormModal(true);
  };

  const handleSaveFormSettings = (e: React.FormEvent) => {
    e.preventDefault();
    if (!activeForm || !editFormTitle.trim()) return;
    updateForm(activeForm.id, {
      title: editFormTitle.trim(),
      programId: editFormProgramId,
      programmeId: editFormProgramId,
      cohortId: editFormCohortId || undefined,
      description: editFormDescription.trim() || undefined,
    });
    setShowEditFormModal(false);
    addToast({
      title: 'Form Settings Saved',
      message: `Updated configuration for "${editFormTitle.trim()}".`,
      type: 'success',
    });
  };
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-200">
      ...
      {/* ========================================================================= */}
      {/* MODAL 6: EDIT FORM SETTINGS & ATTACHMENT MODAL */}
      {/* ========================================================================= */}
      {showEditFormModal && activeForm && (
        <div className="fixed inset-0 z-50 bg-slate-900/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-lg w-full shadow-2xl border border-slate-200 overflow-hidden">
            <div className="bg-slate-900 px-6 py-4 text-white flex items-center justify-between">
              <div className="flex items-center space-x-2">
                ...
                <h3 className="text-base font-bold font-['Space_Grotesk']">
                  Form Settings & Programme Linkage
                </h3>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={handleSaveFormSettings} className="p-6 space-y-4 text-xs">
              <div>
                <label className="block font-bold text-slate-700 uppercase mb-1">
                  Form Title <span className="text-rose-500">*</span>
                </label>
                <input
                  ...
                  placeholder="e.g. Generative AI Intake Assessment"
                  className="w-full px-3.5 py-2.5 border border-slate-300 rounded-xl focus:ring-2 focus:ring-indigo-500 bg-white text-slate-900 font-semibold"
                />
              </div>

              <div>
                <label className="block font-bold text-slate-700 uppercase mb-1">
                  Attached Programme <span className="text-rose-500">*</span>
                </label>
                <select
                  value={editFormProgramId}
                  onChange={e => {
                    setEditFormProgramId(e.target.value);
                    setEditFormCohortId('');
                  }}
                  className="w-full px-3.5 py-2.5 border border-slate-300 rounded-xl focus:ring-2 focus:ring-indigo-500 bg-white text-slate-900 font-medium"
                >
                  {programs.map(p => (
                    <option key={p.id} value={p.id}>
                      {p.name} ({p.code})
              ...
              <div>
                ...
                <select
                  value={editFormCohortId}
                  onChange={e => setEditFormCohortId(e.target.value)}
                  className="w-full px-3.5 py-2.5 border border-slate-300 rounded-xl focus:ring-2 focus:ring-indigo-500 bg-white text-slate-900 font-medium"
                >
                  <option value="">All Cohorts for this Programme (Global default)</option>
                  {cohorts
                    .filter(c => c.programId === editFormProgramId || (c as any).programmeId === editFormProgramId)
                    .map(c => (
              ...
              <div>
                <label className="block font-bold text-slate-700 uppercase mb-1">
                  Form Description / Candidate Guidance
                </label>
                <textarea
                  rows={3}
                  value={editFormDescription}
                  onChange={e => setEditFormDescription(e.target.value)}
                  placeholder="Provide instructions displayed at the top of the applicant portal form..."
                  className="w-full px-3.5 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-indigo-500 bg-white text-slate-900"
                />
              </div>
